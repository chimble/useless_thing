...
def get_pairs(list_names):
    ...
    while i < len(list_names):
        ...
        if element == recipient:
            # Uncomment this line if you'd like to see who picked own name
...
def secret_santa():
    # Uncomment these lines to run classic version:
    ...
    # Comment these lines out if you run the classic version

    fail_list = []
    for i in range(100000):
        pairs, fail_count = get_pairs(['a', 'b', 'c', 'd', 'e',
                                      'f', 'g', 'h', 'i', 'j'])
